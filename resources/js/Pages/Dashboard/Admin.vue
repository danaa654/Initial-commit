...
// General Education, same pattern as the Total Faculty card on
// Teaching Assignments' Index.vue. -1 means "show the grand total";
// clicking walks forward through department buckets and wraps back.
...
const facultyCardView = computed(() => {
    if (facultyViewIndex.value === -1) {
        return { label: 'Faculty Members', count: props.stats.faculty_members, caption: 'Total Faculty' }
    }
...
// Buckets mirror the standard load-policy tiers from project history:
// Full Load 18+, Near Full 15-17, Underloaded <15 (but >0), No Load 0.
...
const scheduleCompletionPercent = computed(() => {
    ...
    return total ? Math.round((scheduled / total) * 100) : 0
})

// Alerts & Notifications — assembled from whatever signal is already
// available (conflict counts + unscheduled subject count). Swap in a
// real widgets.alerts array from DashboardService once one exists.
const alerts = computed(() => {
    const c = props.widgets.conflicts ?? {}
...
<template>
    <DashboardLayout>
        <DashboardBackgroundFX>
            ...
            <!-- Stat cards — your existing StatCard component -->
            <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
                <StatCard label="Departments" :value="stats.departments" glow-color="#8b5cf6">Total Departments</StatCard>
                <StatCard label="Programs" :value="stats.programs" glow-color="#22c55e">Total Programs</StatCard>
                <button
                    type="button"
                    ...
                    style="border-color: color-mix(in srgb, #3b82f6 35%, var(--card-border))"
                    @click="cycleFacultyView"
                >
                    <div class="absolute top-0 left-0 right-0 h-1 opacity-80 group-hover:opacity-100 transition-opacity duration-300" style="background: #3b82f6"></div>
                    <div class="pointer-events-none absolute inset-0 hidden rounded-xl opacity-40 transition-opacity duration-300 group-hover:opacity-90 dark:block" style="box-shadow: inset 0 0 0 1px color-mix(in srgb, #3b82f6 45%, transparent), 0 0 22px 0 color-mix(in srgb, #3b82f6 35%, transparent)"></div>
                    <div class="flex items-center justify-between">
                        <h2 class="text-[var(--text-secondary)] text-xs truncate">{{ facultyCardView.label }}</h2>
                        <span class="w-1.5 h-1.5 rounded-full shrink-0 transition-transform duration-300 group-hover:scale-125" style="background: #3b82f6"></span>
                    </div>
                    ...
                    <div class="mt-1.5 flex items-center gap-1">
                        <span
                            ...
                            :class="(n - 2) === facultyViewIndex ? 'w-3 bg-blue-500' : 'w-1 bg-[var(--card-border)]'"
                        ></span>
                    </div>
                </button>
                <StatCard label="Active Rooms" :value="stats.active_rooms" glow-color="#f97316">Active Rooms</StatCard>
                <StatCard label="Subject Offerings" :value="stats.subject_offerings" glow-color="#14b8a6">This Term</StatCard>
                <StatCard label="Published Schedules" :value="stats.published_schedules" accent="text-pink-500" glow-color="#ec4899">Published</StatCard>
            </div>
